...
/**********************************************************************
  程序名称/Program name     : TCA Thermal conductivity identification
  作者/Author              : Yunsong Zhang 
  日期/Date（YYYYMMDD）     : 2025/1/13
  程序目的/Purpose          :
  辨识TCA的导热率$λ$, 具体实验细节则是,固定TCA然后通过输入功率 并测量当前温度与功率 ,并测量记录.
Identify the thermal conductivity of the TCA $λ $, the details of the experiment is, fixed TCA and then through the input power, 
and measure the current temperature and power, and measurement records.
